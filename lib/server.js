...
const { sanitizeModel, CLAUDE_PRESETS, CURSOR_PRESETS, CODEX_PRESETS, OPENCODE_PRESETS } = require('./models');
...
function maskSettings(s) {
  ...
  if (out.github?.token) out.github.token = out.github.token.slice(0, 10) + '***';
  if (out.github?.webhookSecret) out.github.webhookSecret = out.github.webhookSecret.slice(0, 10) + '***';
  return out;
}
...
function createServer({ getStatus, restartPlatforms, bot } = {}) {
  const app = express();
  app.disable('x-powered-by');

  // GitHub webhook signature verification requires raw body
  ...
  app.post('/api/projects', (req, res) => {
    ...
    if (!['claude', 'cursor', 'codex', 'opencode'].includes(agent)) return res.status(400).json({ error: 'agent must be "claude", "cursor", "codex", or "opencode"' });
  ...
  app.put('/api/projects/:id', (req, res) => {
    ...
    if (agent && !['claude', 'cursor', 'codex', 'opencode'].includes(agent)) return res.status(400).json({ error: 'agent must be "claude", "cursor", "codex", or "opencode"' });
  ...
  app.get('/api/models', (_req, res) => res.json({ claude: CLAUDE_PRESETS, cursor: CURSOR_PRESETS, codex: CODEX_PRESETS, opencode: OPENCODE_PRESETS }));
  ...
  app.put('/api/settings', (req, res) => {
    ...
    if (patch.github?.token?.includes('***')) {
      if (patch.github) patch.github.token = existing.github?.token || '';
    }
    if (patch.github?.webhookSecret?.includes('***')) {
      if (patch.github) patch.github.webhookSecret = existing.github?.webhookSecret || '';
    }
  ...
  // ── GitHub webhook ──────────────────────────────────────────────────────────
  let githubWebhookHandler = null;
  try {
    const { createBot: createGitHubBot } = require('../platforms/github');
    const { webhookHandler } = createGitHubBot();
    githubWebhookHandler = webhookHandler;
  } catch (err) {
    log.debug('github bot creation', { err: err.message });
  }
  ...
  app.get('/api/platforms', (_req, res) => {
    if (typeof bot?.getPlatformStatus === 'function') {
      return res.json(bot.getPlatformStatus());
    }
    res.status(503).json({ error: 'Platform status not available' });
  });

  app.post('/api/platforms/:name/start', (req, res) => {
    if (typeof bot?.startPlatform === 'function') {
      bot.startPlatform(req.params.name);
      return res.json({ ok: true, platform: req.params.name, action: 'start' });
    }
    res.status(503).json({ error: 'Platform control not available' });
  });

  app.post('/api/platforms/:name/stop', (req, res) => {
    if (typeof bot?.stopPlatform === 'function') {
      bot.stopPlatform(req.params.name);
      return res.json({ ok: true, platform: req.params.name, action: 'stop' });
    }
    res.status(503).json({ error: 'Platform control not available' });
  });
