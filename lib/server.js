...
function createServer({ getStatus, restartBot } = {}) {
  const app = express();
  ...
  app.put('/api/settings', (req, res) => {
    ...
    const telegramChanged = patch.telegram !== undefined;

    try {
      const updated = saveSettings(patch);
      reloadConfig();
      if (telegramChanged && typeof restartBot === 'function') {
        setImmediate(restartBot);
      }
