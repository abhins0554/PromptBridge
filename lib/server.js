...
const { sanitizeModel, CLAUDE_PRESETS, CURSOR_PRESETS, CODEX_PRESETS } = require('./models');
...
function createServer({ getStatus, restartPlatforms, bot } = {}) {
  ...
  app.post('/api/projects', (req, res) => {
    ...
    if (!['claude', 'cursor', 'codex'].includes(agent)) return res.status(400).json({ error: 'agent must be "claude", "cursor", or "codex"' });
  ...
  app.put('/api/projects/:id', (req, res) => {
    ...
    if (agent && !['claude', 'cursor', 'codex'].includes(agent)) return res.status(400).json({ error: 'agent must be "claude", "cursor", or "codex"' });
  ...
  app.get('/api/models', (_req, res) => res.json({ claude: CLAUDE_PRESETS, cursor: CURSOR_PRESETS, codex: CODEX_PRESETS }));
