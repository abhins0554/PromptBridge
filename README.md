...
Drive **Claude Code**, **Cursor**, **Codex**, and **OpenCode** agents from **Telegram**, **Discord**, **Slack**, **Teams**, **GitHub**, **Email**, or any platform you wire in — with a zero-config web dashboard.
...
Supports four agents out of the box: **Claude Code**, **Cursor**, **OpenAI Codex**, and **OpenCode**.
...
<pre><code>You (Telegram / Discord / Slack / Teams / GitHub / Email)
 ...
 (claude CLI) (cursor-agent) (codex CLI) (opencode CLI)
</code></pre>
</div>
...
- **GitHub integration** — mention `/claude`, `/cursor`, `/codex`, or `/opencode` in issue/PR comments, bot responds as comments
- **Email triggers** — send `hi /claude <prompt>`, `hi /cursor <prompt>`, `hi /codex <prompt>`, or `hi /opencode <prompt>` to your inbox, get a reply with results
...
- **OpenCode CLI** (`opencode`) on PATH — optional, for OpenCode support
...
Plain text (no slash) runs the active project's agent. Send a file with a `/claude`, `/cursor`, `/codex`, or `/opencode` caption to have the agent read it.
...
Plain channel messages run the active project's agent. Send attachments with a `/claude`, `/cursor`, `/codex`, or `/opencode` message to have the agent read them from disk.
...
Mention `/claude`, `/cursor`, `/codex`, or `/opencode` in **issue comments** or **pull request comments** to trigger the agent. The bot responds directly to the same issue/PR via polling (every 120 seconds):
...
| `/opencode <prompt>` | Ask OpenCode in an issue/PR comment |
| `/help` | Show available commands |

**Example:** In a pull request comment, type:
```
/opencode please analyze this code and suggest improvements
```

The bot will:
1. Poll the repo every 2 minutes for your `/claude`, `/cursor`, `/codex`, or `/opencode` mentions
...
**Session awareness:** The bot maintains conversation state per issue/PR, so follow-up commands in the same thread include all previous context.
...
hi /opencode explain the architecture of this codebase
...
Supported triggers: `hi /claude`, `hi /cursor`, `hi /codex`, `hi /opencode`
...
| OpenCode executable | Path to `opencode` CLI (default: `opencode`) |
...
platforms/
  ...
  github/
    ...
    polling.js                GitHub polling agent — polls allowed repos for /claude /cursor /codex /opencode commands
...
lib/
  ...
  runner.js                   Spawns claude / cursor-agent / codex / opencode CLI processes
  ...
  models.js                   Model presets (Claude / Cursor / Codex / OpenCode)
...
| `GET` | `/api/models` | Required | Claude + Cursor + Codex + OpenCode model presets |
